---
tags: [langchain, runnable, lcel, python, few-shot]
til: v2 2026-09-01
---

# LangChain 기초 — Runnable, LCEL 파이프라인, 실습 4종
> 작성일: 2026-09-01

## 🔗 관련 글

- [2026-08-03 CS기본 지식](2026-08-03%20CS기본%20지식(소프트웨어공학,%20하드웨어,%20네트워크%20등).md) — LangChain·LangGraph가 "AI 시대의 신규 프레임워크"로 처음 언급된 부분의 뒷부분
- [2026-08-27 (3) Gemini 비동기 요청 실습](2026-08-27%20(3)%20Gemini%20비동기%20요청%20실습.md) — `client`(재사용 통로)와 개념적으로 이어지는 `Runnable` 규격화의 뒷부분
...
1. LangChain이 "규격을 통일한다"는 것은 구체적으로 무엇을 통일한다는 뜻인가?
2. `invoke()` 인터페이스가 같다는 것과 데이터 타입이 같다는 것은 왜 다른 이야기인가?
3. `prompt | llm | parser`처럼 순서가 고정되는 이유는 무엇인가?
4. `a | b`가 실제로는 어떤 코드로 번역되는가? 이게 왜 import 없이 동작하는가?
5. `|` 기호를 파이프 연산자로 채택한 이유(관례)는 무엇인가?
6. 일반 파이썬 함수를 파이프라인에 넣으려면 왜 `RunnableLambda`로 감싸야 하는가?
7. `RunnablePassthrough.assign()`에서 실제로 값을 채우는 주체는 누구인가?
8. `batch()`와 `asyncio.gather()`는 기능이 같은데 왜 굳이 나뉘어 있는가?
9. `stream()`이 토큰을 아껴주지 않는데도 쓰는 이유는 무엇인가?
10. few-shot 예시가 학습 데이터가 아니라는 것은 무슨 뜻이며, 왜 새로운 입력에도 일반화되는가?
