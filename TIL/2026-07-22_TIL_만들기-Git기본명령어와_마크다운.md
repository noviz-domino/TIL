...
2. git push -u origin main을 딱 한 번만 쓰면 되는 이유는? -u가 다시 필요한 경우 3가지는?
3. origin 연결은 폴더 단위인가 컴퓨터 전체 단위인가? 그게 왜 위험 관리에 중요한가?
4. git push 전에 pwd와 git remote -v를 먼저 확인하는 습관이 막아주는 실수는 무엇인가?
5. 마크다운에서 인라인 코드와 코드 블록의 문법 차이는?
6. 마크다운 이미지 문법과 링크 문법의 차이는?
